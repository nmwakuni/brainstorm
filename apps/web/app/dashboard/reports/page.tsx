...
export default function ReportsPage() {
  const { data: _dashboardData } = useQuery({
    queryKey: ['dashboard'],
  ...
  const metrics = {
    ...
    totalAdvanced:
      advancesData?.advances?.reduce((sum: number, adv: any) => sum + parseFloat(adv.amount), 0) ||
      0,
    totalFees:
      advancesData?.advances?.reduce((sum: number, adv: any) => sum + parseFloat(adv.fee), 0) || 0,
    averageAdvance: advancesData?.advances?.length
      ? advancesData.advances.reduce((sum: number, adv: any) => sum + parseFloat(adv.amount), 0) /
        advancesData.advances.length
      : 0,
    disbursedCount:
      advancesData?.advances?.filter((a: any) => a.status === 'disbursed').length || 0,
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">Insights and metrics for your salary advance program</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <div className="card">
          ...
          <p className="text-sm text-gray-500 mt-2">Across {metrics.totalAdvances} requests</p>
        </div>

        <div className="card">
          <h3 className="text-sm text-gray-600 mb-1">Total Fees Collected</h3>
          <p className="text-3xl font-bold text-green-600">{formatCurrency(metrics.totalFees)}</p>
          <p className="text-sm text-gray-500 mt-2">4% platform fee</p>
        </div>
        ...
        <div className="card">
          ...
          <p className="text-sm text-gray-500 mt-2">Per request</p>
        </div>

        <div className="card">
          <h3 className="text-sm text-gray-600 mb-1">Utilization Rate</h3>
          <p className="text-3xl font-bold text-purple-600">{utilizationRate.toFixed(1)}%</p>
        ...
        <div className="card">
          ...
          <p className="text-3xl font-bold text-green-600">{metrics.disbursedCount}</p>
        ...
        <div className="card">
          ...
          <p className="text-3xl font-bold text-yellow-600">{metrics.pendingCount}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          {advancesData?.advances && advancesData.advances.length > 0 ? (
            <div className="space-y-3">
              {[
                ...
                const count = advancesData.advances.filter((a: any) => a.status === status).length
        ...
        {/* Top Requesters */}
        <div className="card">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Most Active Employees</h2>
          {advancesData?.advances && advancesData.advances.length > 0 ? (
            <div className="space-y-3">
              {Object.entries(
